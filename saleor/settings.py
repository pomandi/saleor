...
# NEXT_PUBLIC_SALEOR_API_URL=https://saleor-5f11a4761db9.herokuapp.com/graphql/
...
ALLOWED_HOSTS = get_list(os.environ.get("ALLOWED_HOSTS", "localhost,127.0.0.1,saleor-5f11a4761db9.herokuapp.com"))
...
BEAT_EXPIRE_ORDERS_AFTER_TIMEDELTA = datetime.timedelta(
    seconds=parse(os.environ.get("BEAT_EXPIRE_ORDERS_AFTER_TIMEDELTA", "45 minutes"))
)
...
OBSERVABILITY_BUFFER_TIMEOUT = datetime.timedelta(
    seconds=parse(os.environ.get("OBSERVABILITY_BUFFER_TIMEOUT", "45 minutes"))
)
...
GRAPHQL_QUERY_MAX_COMPLEXITY = int(
    os.environ.get("GRAPHQL_QUERY_MAX_COMPLEXITY", 5000000)
)
...
JWT_TTL_ACCESS = datetime.timedelta(
    seconds=parse(os.environ.get("JWT_TTL_ACCESS", "45 minutes"))
)
JWT_TTL_APP_ACCESS = datetime.timedelta(
    seconds=parse(os.environ.get("JWT_TTL_APP_ACCESS", "45 minutes"))
)
...
TOKEN_UPDATE_LAST_LOGIN_THRESHOLD = parse(
    os.environ.get("TOKEN_UPDATE_LAST_LOGIN_THRESHOLD", "45 seconds")
)
...
CORS_ALLOW_ALL_ORIGINS = True  # Geliştirme için, prod'da sadece frontend domainini eklemen önerilir
