...
class GraphQLView(View):
    ...
    def handle_query(self, request: HttpRequest) -> JsonResponse:
        # Gelen isteğin detayını logla
        logger = logging.getLogger("saleor.graphql.request")
        logger.info(f"GraphQL Request Headers: {dict(request.headers)}")
    ...
    def execute_graphql_request(self, request: HttpRequest, data: dict):
        with opentracing.global_tracer().start_active_span("graphql_query") as scope:
            ...
            # Log the query string before attempting to parse it
            logger = logging.getLogger("saleor.graphql.request")
            logger.info(f"!!! EXECUTE_GRAPHQL !!! Query before parse_query: '{query}'")
    ...
    def parse_body(request: HttpRequest):
        logger = logging.getLogger("saleor.graphql.request") # Logger'ı başta tanımlayalım
        content_type = request.content_type
        logger.info(f"!!! PARSE_BODY START !!! Content-Type: {content_type}")

        if content_type == "application/graphql":
            try:
                query_bytes = request.body
                logger.info(f"!!! PARSE_BODY [graphql] !!! Raw Bytes Length: {len(query_bytes)}")
                query_str = query_bytes.decode("utf-8")
                logger.info(f"!!! PARSE_BODY [graphql] !!! Decoded Query: '{query_str}'")
                return {"query": query_str}
            except Exception as e:
                logger.error(f"!!! PARSE_BODY [graphql] !!! Decode Error: {e}")
                raise ValueError("Unable to decode application/graphql body.")

        if content_type == "application/json":
            try:
                body_bytes = request.body
                logger.info(f"!!! PARSE_BODY [json] !!! Raw Bytes Length: {len(body_bytes)}")
                body_str = body_bytes.decode("utf-8")
                logger.info(f"!!! PARSE_BODY [json] !!! Decoded Body String: '{body_str}'")
                # json.loads'u try-except içine alalım
                try:
                    parsed_data = json.loads(body_str)
                    logger.info("!!! PARSE_BODY [json] !!! JSON Loaded Successfully.")
                    return parsed_data
                except json.JSONDecodeError as e:
                    logger.error(f"!!! PARSE_BODY [json] !!! json.loads Error: {e}. Body was: '{body_str}'")
                    # ValueError yükseltmek _handle_query'deki except bloğunu tetikler
                    raise ValueError(f"Unable to parse JSON: {e}")
            except Exception as e:
                # Decode hatası veya başka beklenmedik hata
                logger.error(f"!!! PARSE_BODY [json] !!! Outer Decode/Processing Error: {e}")
                raise ValueError("Unable to process application/json body.")

        if content_type in ["application/x-www-form-urlencoded", "multipart/form-data"]:
            logger.info(f"!!! PARSE_BODY [form] !!! Returning request.POST")
            return request.POST

        logger.warning(f"!!! PARSE_BODY !!! Unknown Content-Type: {content_type}. Returning empty dict.")
        return {}
